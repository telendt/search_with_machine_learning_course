...
class DataPrepper:
    ...
    def __log_ltr_query_features(self, query_id, key, query_doc_ids, click_prior_query, no_results, terms_field="_id"):
        ...
        resp = self.opensearch.search(log_query, index=self.index_name)
        ...
        # # capture all query/doc pairs that didn't return features
        # missing = set(query_doc_ids).difference(feature_results["doc_id"])
        # if missing:
        #     feature_missing = {
        #         "doc_id": missing,
        #         "query_id": [query_id] * len(missing),
        #         "sku": missing
        #     }
        #     nan_fill = [None] * len(missing)
        #     for key in (feature_missing.keys() | feature_results.keys()):
        #         feature_results[key].extend(feature_missing.get(key, nan_fill))
    ...
    def __num_clicks(self, all_skus_for_query, test_sku):
        return (all_skus_for_query == test_sku).sum()
